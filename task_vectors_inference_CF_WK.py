...
# Define an editing function for modifying hidden states
...
def count_flips(task, edit_answers, original_answers, true_answers):
    ...
    print("Eligible Flip to World-Knowledge %: ", flip_to_correct_count/original_not_true)
...
def main():     
    ...
    if args.task == "color":
        ...
        if model_name == "llava-next":
            ...
            df = pd.read_csv("color_qwen_filtered_CF.csv")
            
        elif model_name == "janus":
            df = pd.read_csv("color_janus_filtered_CF.csv") #.head(5)
    ...
    for idx, row in df.iterrows(): 
        if args.task == "color":
            true_answers[row['correct_object']] = row['correct_answer'] #row[f'{args.model_version}_lm_preds'].lower().replace("_", " ").replace(".", "") #row['incorrect_answer'] #correct_answer 
        else:
            key = row['correct_object'] + " vs. " + row['comparison_object']
            true_answers[key] = row['correct_answer']
